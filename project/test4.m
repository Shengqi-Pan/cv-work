...
% 根据半径框出ROI
RecLDown = [floor(YCenter - R / 2), floor(XCenter - R / 4)];%矩形左下角的点为RecLDown
RecRDown = [floor(YCenter - R / 2), floor(XCenter + R / 4)];%矩形右下角的点为RecRDown
RecLUp = [max(1, floor(YCenter - R / 2 - R)), floor(XCenter - R / 4)];%矩形左上角的点为RECLUp
RecRUp = [max(1, floor(YCenter - R / 2 - R)), floor(XCenter + R / 4)];%矩形右上角的点为RecRUp
...
figure;
Finalrgbimg = rgbimg;
Finalrgbimg(YPlaceU : YPlaceD, XPlaceL-15 : XPlaceR-15, 1) = 256;
Finalrgbimg(YPlaceU : YPlaceD, XPlaceL-15 : XPlaceR-15, 2 : 3) = 1;
imshow(Finalrgbimg);
% 转换到原坐标下并作图
